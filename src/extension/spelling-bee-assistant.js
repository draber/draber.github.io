(function () {
    ...
    const get$1 = (data, table = null, markCompleted = true, highlightMarker = '') => {
        ...
        data.forEach((rowData, i) => {
            ...
            if(markCompleted && i > 0 && completable && rowData.indexOf(0) === position){
                classNames.push(prefix$1('complete', 'd'));
            }
            if(highlightMarker && rowData.includes(highlightMarker)) {
                classNames.push(prefix$1('highlight', 'd'));
            }
            const tr = el.tr({
