(function () {
    ...
    class Widget {
        getState() {
            const stored = settings$1.get(`options.${this.key}.enabled`);
            return typeof stored !== 'undefined' ? stored : this.defaultState;
        }
        setState(state) {
            if (this.canChangeState) {
                settings$1.set(`options.${this.key}.enabled`, state);
            }
            return this;
        }
        toggle(state) {
    ...
    class ColorConfig extends Plugin {
        toggleColorParameters(parameters) {
            ...
            console.log('toggleColorParameters',settings$1.get(`options.${this.key}`));
        ...
        getColorParameters() {
            return {
                hue: settings$1.get(`options.${this.key}.hue`) || 0,
                sat: settings$1.get(`options.${this.key}.sat`) || 0,
            };
        }
        setColorParameters(parameters) {
            settings$1.set(`options.${this.key}`, parameters);
            console.log('setColorParameters', settings$1.set(`options.${this.key}`));
            return this;
        }
        togglePopup() {
    ...
    const add = (shortcut) => {
        ...
        const overrides = settings$1.get(`shortcuts.${shortcut.module}.${shortcut.combo}`) || {};
